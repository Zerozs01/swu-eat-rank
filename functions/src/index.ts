...
export const deleteMenuImage = onRequest({ region: 'asia-east1', cors: [ 'https://swu-eat-rank.web.app', 'https://swu-eat-rank.firebaseapp.com', 'http://localhost:5173' ] }, async (req: FnRequest, res: Response) => {
  const method = req.method as unknown as string;
  if (method === 'OPTIONS') {
    res.status(204).end();
    return;
  ...
  try {
    ...
    await file.delete({ ignoreNotFound: true } as any).catch(async (err: any) => {
